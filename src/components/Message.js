...
function Message({id, message, timestamp, name, email, photoURL}) {
    ...
    <div className='flex items-center p-1 pl-5 my-5 mr-2 hover:bg-discord_messageBg group'>
        ...
        {user?.email === email && (
            <div className='hover:bg-discord_red p-1 ml-auto rounded-sm text-discord_red hover:text-white cursor-pointer' onClick={() => db.collection("channels").doc(channelId).collection("messages").doc(id).delete()}>
